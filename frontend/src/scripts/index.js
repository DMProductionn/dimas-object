...
const initModal = () => {
  ...
  document.addEventListener('click', function (e) {
    if (
      ...
      !e.target.closest('.programm__card-button') &&
      !e.target.closest('.programm__button') &&
      !e.target.closest('.why__reboot-button') &&
      !e.target.closest('.steamRoom')
    ) {
  ...
  programmsMobile.forEach((program) => {
    program.addEventListener('click', () => {
      ...
      const currentContainer =
        program.querySelector('.programm__cards') || program.nextElementSibling;
      if (currentContainer) {
